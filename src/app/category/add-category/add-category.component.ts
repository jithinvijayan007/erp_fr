...
// settings defines the data structure of smart table
 settings = {
  ...
  // function lisData lists all categories
listData () {
  this.serviceObject.getData('category/categorylist/').subscribe(res => {
    console.log(res);
    res['lst_articles'].forEach((element, index) => {
      element['slno'] = index + 1;
    });
    this.data = res['lst_articles'];
    this.source = new LocalDataSource(this.data); // create the source
  });
}
// function saveData save the category details
saveData() {
const dct_data = {};
if (this.vchr_code === '') {
  this.toastr.error('Enter category code');
  return;
} else if (this.vchr_name === '') {
  this.toastr.error('Enter category name');
  return;
}
dct_data['vchr_name'] = this.vchr_name;
dct_data['vchr_code'] = this.vchr_code;

this.serviceObject.postData('category/categoryadd/', dct_data)   .subscribe(
  (response) => {
...
// function updateRecord updates any change in category details
updateRecord(event) {
    this.serviceObject.putData('category/categoryadd/', event.newData).subscribe(
      (response) => {
          if (response.status === 1) {
          ...
          this.listData();
            ...
            this.toastr.error('Category updation failed');
          }
      },
      (error) => {
  ...
  deleteRecord(event) {
    this.serviceObject.patchData('category/categoryadd/', event.data ).subscribe(
      (response) => {
        if (response.status === 1) {
