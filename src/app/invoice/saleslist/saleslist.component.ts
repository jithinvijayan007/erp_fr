...
export class SaleslistComponent implements OnInit {
  ...
  onCustomAction(event)
  {
    ...
    if(event.data.blnSez){
      console.log("gdfgdfgfdg");
      
      localStorage.setItem('previousUrl','invoice/sezinvoice');
      this.router.navigate(['invoice/sezinvoice']);
    }
    else if((event.data.sales_status == 3) ||(event.data.sales_status == 4)){
