...
@Component({
  selector: 'app-recharge-profit-report',
  templateUrl: './recharge-profit-report.component.html',
  styleUrls: ['./recharge-profit-report.component.css']
})
export class RechargeProfitReportComponent implements OnInit {
  ...
  displayedColumns=['type','qty','packageAmount','packageProfit','discount','indirectDiscount','tax','netProfit'];
  ...
  getData()
  {
    ...
    if(this.lstBranch.length>0){
      ...
      this.serviceObject.postData('reports/recharge_profit_report/',dctData).subscribe(
        (response) => {
