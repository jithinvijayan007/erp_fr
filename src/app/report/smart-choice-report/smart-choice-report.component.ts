...
@Component({
  selector: 'app-smart-choice-report',
  templateUrl: './smart-choice-report.component.html',
  styleUrls: ['./smart-choice-report.component.css']
})
export class SmartChoiceReportComponent implements OnInit {
  ...
  displayedColumns=['branch','brand','item','quantity','imei','sold_value','purchase_value','profit'];
  ...
  ngOnInit() {
    ...
    this.lstPermission.forEach(item=> {
      ...
      if (item["NAME"] == "Smart Choice Report" && item["PARENT"] == "PROFIT REPORT") {
  ...
  getData()
  {
    ...
    if(this.lstBranch.length>0){
      ...
      this.serviceObject.postData('reports/smart_choice_report/',dctData).subscribe(
        (response) => {
