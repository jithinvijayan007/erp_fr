...
// import * as XLSXtyle from 'xlsx-style';
...
export class ExcelService {
  ...
  public exportAsEnquiryExport(json: any[], expjsondata): void {
    ...
    if (expjsondata['staff'] == '') { expjsondata['staff'] = 'ALL' }
    if (expjsondata['customer'] == '') { expjsondata['customer'] = 'ALL' }
    if (expjsondata['brand'] == '') { expjsondata['brand'] = 'ALL' }
    ...
    if (expjsondata.hasOwnProperty('blnSalesDashboard') && expjsondata['blnSalesDashboard']) {
      this.headerCenterStyle(ws.A1);
      this.headerCenterStyle(ws.A2);
      this.wrapAndCenterCell(worksheet.A10);
      this.wrapAndCenterCell(worksheet.B10);
  ...
  public exportAsEnqOnlyExport(json: any[], expjsondata): void {
    ...
    console.log(json, "json laaaaaaaaaaaaaaaa", expjsondata);
    // console.log(localStorage.getItem("username"),"Username");
    expjsondata['user'] = localStorage.getItem("username");
    // console.log(expjsondata['charthead'],expjsondata['tmpdfdate'],expjsondata,"this the expjsondata arecieved at excelservice");
    if (expjsondata['branch'] == '') { expjsondata['branch'] = 'ALL' }
    ...
    if (expjsondata['component'] == 'NA_Report') {
      ...
      for (let column in displayColumn) {
        ...
        if (displayColumn[column].toLowerCase() == 'product') {

          if (expjsondata['product']) {
            expjsondata[num] = expjsondata['product'];
          }
          else {
            expjsondata[num] = 'ALL';
          }
        }
        if (displayColumn[column].toLowerCase() == 'brand') {

          if (expjsondata['brand']) {
            expjsondata[num] = expjsondata['brand'];
          }
          else {
            expjsondata[num] = 'ALL';
          }
      ...
      else if (displayColumn.length == 3) {
  ...
  private wrapAndCenterCell(cell: XLSX.CellObject) {
    ...
    // console.log(cell,wrapAndCenterCellStyle);
  ...
  private totalCellStyle(cel: XLSX.CellObject) {
    const totStyle = { font: { sz: 11, bold: true }, alignment: { wrapText: true, vertical: 'center', horizontal: 'center' } };
  ...
  /// recharge report start

  exportAsRechargeReport(json: any[], expjsondata): void {
    ...
    if (expjsondata['charthead'] == "Recharge Report") {
      var ws = XLSX.utils.aoa_to_sheet([
        [heading], [""], [""],
        ["Taken By:", expjsondata['user']],
        ["Action Date:", mydate],
        ["Date:", [timeperiod]],
        ["Branch:", expjsondata['branch']],
        ['Recharge Type:', expjsondata['recharge']]
  ...
  // recharge report ends


  exportAsServiceReport(json: any[], expjsondata): void {
    ...
    let heading = expjsondata['charthead'];
    let timeperiod = expjsondata['dateFrom'] + '-' + expjsondata['dateTo'];

    if (expjsondata['charthead'] == "Service Report") {
      var ws = XLSX.utils.aoa_to_sheet([
        [heading], [""], [""],
        ["Taken By:", expjsondata['user']],
        ["Action Date:", mydate],
    ...
    this.wrapAndCenterCell(worksheet.H10);
    this.wrapAndCenterCell(worksheet.I10);
    this.wrapAndCenterCell(worksheet.J10);
    this.wrapAndCenterCell(worksheet.K10);
    this.wrapAndCenterCell(worksheet.L10);
  ...
  exportAsEnquiryReport(json: any[], expjsondata): void {
  ...
  exportAsPromoterReport(json: any[], expjsondata): void {
    ...
    var ws = XLSX.utils.aoa_to_sheet([
      [heading], [""], [""],
      ["Taken By:", expjsondata['user'], [""], [""], "Brand:", expjsondata['Brand']],
      ["Action Date:", mydate, [""], [""], "Zone:", expjsondata['Zone']],
      ["Date:", [timeperiod], [""], [""], "Territory:", expjsondata['Territory']],
      ["Branch:", expjsondata['Branch']]
    ...
    this.wrapAndCenterCell(worksheet.K10);
    this.wrapAndCenterCell(worksheet.L10);
    this.wrapAndCenterCell(worksheet.M10);
    this.wrapAndCenterCell(worksheet.N10);
  ...
  exportAsTargetComparisonReport(json: any[], expjsondata): void {
    ...
    let heading = expjsondata['heading'];
    // let timeperiod=expjsondata['dateFrom'] +'-' + expjsondata['dateTo'];
    ...
    var ws = XLSX.utils.aoa_to_sheet([
      ...
      ["Taken By:", expjsondata['user']],
      ["Action Date:", mydate],
      ["Selected Zones:", expjsondata['zones']],
      ["Selected Territories:", expjsondata['territory']],
      ["Selected Branches:", expjsondata['branch']],
    ]);
    ...
    const opts: any = { sheetFormat: { baseColWidth: '200' }, };
    const excelBuffer: any = XLSX.write(workbook, { bookType: 'xlsx', type: 'array' });
    this.saveAsExcelFile(excelBuffer, expjsondata['heading']);
  ...
  exportAsProfitReport(json: any[], expjsondata): void {

    // console.log("@exportAsProfitReport json",json,Object.keys(json[0]).length);
    // console.log("@exportAsProfitReport expjsondata",expjsondata);
    ...
    if (Object.keys(json[0]).length > 6) //Exporting table data
      this.wrapAndCenterCell(worksheet.G10);

    if (Object.keys(json[0]).length > 7) //Exporting table data
      this.wrapAndCenterCell(worksheet.H10);
    // this.wrapAndCenterCell(worksheet.I10);
    // this.wrapAndCenterCell(worksheet.J10);
    // this.wrapAndCenterCell(worksheet.K10);
    ...
    let colLength = Object.keys(json[0]).length;
    let colStart = 2;

    if (Object.keys(json[0]).length == 6) //For export inside eye button
      colStart = 1;

    if (Object.keys(json[0]).length == 8)
      colStart = 3;

    for (let j = 10; j < num - 1; j++) {
      for (let i = colStart; i < colLength; i++) {
  ...
  exportAsDiscountReport(json: any[], expjsondata): void {
    ...
    let heading = expjsondata['charthead'];
    // let timeperiod=expjsondata['dateFrom'] +'  to  ' + expjsondata['dateTo'];

    var ws = XLSX.utils.aoa_to_sheet([
      [heading], [""], [""],
      ["Taken By:", expjsondata['user']],
      ["Action Date:", mydate],
      ["From Date:", [expjsondata['dateFrom']]],
      ["To Date:", [expjsondata['dateTo']]],
    ]);
  ...
  exportAsSalesComparisonReport(json: any[], expjsondata): void {
    ...
    // if(expjsondata['branch']==''){expjsondata['branch']='ALL'}
    if (expjsondata['branch'] == '') { expjsondata['branch'] = 'ALL' }
    if (expjsondata['itemGroup'] == '') { expjsondata['itemGroup'] = 'ALL' }
    if (expjsondata['brand'] == '') { expjsondata['brand'] = 'ALL' }
    if (expjsondata['product'] == '') { expjsondata['product'] = 'ALL' }

    let heading = expjsondata['chartHead'];
    let timeperiod = expjsondata['tmpdfdate'] + ' to ' + expjsondata['tmpdtdate']

    if (expjsondata['component'] == "Sales_Comparison_Report") {

      var ws = XLSX.utils.aoa_to_sheet([
        [heading], [""], [""], [""],
        ["Branch:", expjsondata['branch'], ["Product:"], expjsondata['product'], "", "", "Taken By:", expjsondata['user']],
        [["Brand:"], expjsondata['brand'], "Item Group:", expjsondata['itemGroup'], "", "", "Reporting Period:", [timeperiod]],
        ["", "", "", "", "", "", "Action Date:", mydate]
    ...
    for (let j = 10; j < num + 1; j++) {
      for (let i = 0; i < 5; i++) {
        var cellRef = XLSX.utils.encode_cell({ r: j, c: i });
        if (i == 0) {
          this.cellTextStyle(worksheet[cellRef]);
  ...
  private cellTextStyle(cel: XLSX.CellObject) {
    const dataStyle = { alignment: { wrapText: true, vertical: 'center', horizontal: 'left' } }
    this, this.setCellStyle(cel, dataStyle);
  }
