...
export class ReceiptOrderListComponent implements OnInit {
  ...
  ngOnInit(){
    ...
    this.lstPermission.forEach(item=> {
      if (item["NAME"] == "Receipt Order List") {
  ...
  getList(startDate, endDate, status){
    ...
    let data;
     if (status === 0) {
      const urls = ['receipt/receipt-order-list']

    //  if (this.previusUrl != '/crm/viewlead'  ) {
      if (!(urls.find( x => x === this.previusUrl))) {
          localStorage.removeItem('enquiryCustomerReceiptStatus')
          localStorage.removeItem('enquiryReceiptRequestData')
     }
     if (localStorage.getItem('enquiryCustomerReceiptStatus')) {
        tempData = JSON.parse(localStorage.getItem('enquiryReceiptRequestData'))
      ...
      localStorage.setItem('enquiryReceiptRequestData', JSON.stringify(data))
    ...
    this.serviceObject.postData('receipt/receipt_order_list/',dctData).subscribe(
      (response) => {
  ...
  {
    localStorage.setItem('editReceiptId',event.data.intId);
    localStorage.setItem('advanceReceiptPayment',String(true));
